...
import { useUnifiedCreateFlexible, useAccountMode } from "@/hooks/useUnifiedContracts"
...
export function FlexibleForm() {
  ...
  const { create, isLoading, isSuccess, hash, poolAddress } = useUnifiedCreateFlexible(
    validMembers,
          ...
          <Button type="submit" className="w-full bg-primary hover:bg-primary/90" disabled={isLoading || isSavingToDB}>
            {isLoading || isSavingToDB ? (
              ...
              <>
                ...
                Create Flexible Pool {isSmartAccountReady && "(Gasless)"}
              </>
            )}
          </Button>
